...
val quicksand = FontFace(
  "Quicksand",
  "url('/dictee-the-game/Quicksand-Bold.ttf') format('truetype')"
)

val alphabetLetters = ('a'.toByte.toInt to 'z'.toByte.toInt).map(_.toByte.toChar).toVector

val alphabet = {
  val letters      = alphabetLetters
  val upperLetters = letters.map(_.toUpper)
  val digits       = (0 to 9).map(_.toString.head).toVector
  val symbols      = Vector('.', '?', '!', ' ', '#', '>', '<', ':', '/', '€', ',', '%', '-', 'é', '\'', 'à')
  Vector(
    letters,
    upperLetters,
    digits,
...
def createGlyphFontData(
...
): Future[(String, dom.ImageData, GlyphInfo)] =
  ...
  def sleep(millis: Long): Future[Unit] = {
    val p = Promise[Unit]()
    scala.scalajs.js.timers.setTimeout(millis.toDouble) {
      p.complete(Success(()))
    }
    p.future
  }
